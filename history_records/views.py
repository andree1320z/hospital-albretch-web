...
def index(request, username):
    try:
        if not (request.user.is_authenticated() and (
                        request.user.profile.user_type == 6 or request.user.profile.user_type == 2)):
    ...
    if user.profile.user_type == 6:
        ...
        rec = Receptionist.objects.get(user='recepcionista1')
        medical_files = patient.medical_file.all().filter(parent_hospital=rec.hospital).order_by('date_of_addmition')
    # if medical_files:

    return render_to_response(request, 'medical_history/medical_files.html',
                              {'patient': patient, 'files': medical_files})
...
def medical_history(request, username, file_id):
    try:
        ...
        query_str = urlencode(d)
        return HttpResponseRedirect('/login_all/?' + query_str)
    if not (request.user.profile.user_type == 6 or request.user.profile.user_type == 2):
    ...
    if request.method == "POST":
        ...
        return render(request, 'medical_history/medical_history_sheet.html',
                      {'patient': patient, 'sheet': history, 'type': type})


def progress_notes(request, username, file_id):
    try:
        if not (request.user.is_authenticated() and (
                        request.user.profile.user_type == 6 or request.user.profile.user_type == 0)):
            d = {'server_message': "Not Logged In."}
            query_str = urlencode(d)
            return HttpResponseRedirect('/login_all/?' + query_str)
    except:
    ...
    try:
        ...
        return render(request, 'medical_history/progress_notes_sheet.html',
                      {'patient': patient, 'sheet': progress_note_sheet, 'type': type})
    except:
...
def physician_orders(request, username, file_id):
    ...
    try:
        ...
        return render(request, 'medical_history/physician_oder_sheet.html',
                      {'patient': patient, 'sheet': orders, 'type': type})
    except:
        return HttpResponse("Currently No Records")
...
def unit_summary(request, username, file_id):
    ...
    try:
        ...
        return render(request, 'medical_history/unit_summary_sheet.html',
                      {'patient': patient, 'sheet': unit_summary_sheet, 'type': type})
    except:
        return HttpResponse("Currently No Records")
...
def edit_physician_orders(request, username, file_id):
    ...
    user = request.user
    phys = Physician.objects.get(user=user)
    new_order = Order(date=request.POST['date'], time=request.POST['time'], order=request.POST['order'], physician=phys,
                      sheet=orders)
...
def edit_progress_notes(request, username, file_id):
    ...
    user = request.user
    phys = Physician.objects.get(user=user)
    new_progress_note = Progress_note(date=request.POST['date'], treatment_progress=request.POST['tp'],
                                      sheet=progress_note_sheet, physician=phys)
    new_progress_note.save()
    return render(request, 'medical_history/progress_notes_sheet.html',
                  {'patient': patient, 'sheet': progress_note_sheet})
...
def create_medical_file(request, username):
    ...
    if request.method == 'POST':
        try:
            medi_file = MedicalFile(parent_hospital=phys.hospital, patient=patient,
                                    date_of_addmition=datetime.datetime.today().date(), open=1,
                                    ward=request.POST['ward'], room=request.POST['room'], bed=request.POST['bed'])
            medi_file.save()
            progress_sheet = Progress_notes_sheet(medical_file=medi_file, date=datetime.datetime.today().date(),

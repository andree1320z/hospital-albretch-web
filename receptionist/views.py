        ...
        if request.GET.get("submit_search_button"):  # if search submit button clicked
            ...
            if ('q' in request.GET) and request.GET['q'].strip():
                ...
                entry_query = get_query(query_string,
                                        ['firstname', 'lastname', 'patient_section', 'patient_room', 'user__username'])
...
#     model = Patient
#     success_url = reverse_lazy('rec_app:patient-list')
#     #pk_url_kwarg = 'username'
...
def set_schedule(request, username):
    ...
    sch = Schedule(patient=patient, event=event, event_date=event_date, event_time=event_time,
                   related_division=related_division, description=description)
    sch.save()
    return sch
